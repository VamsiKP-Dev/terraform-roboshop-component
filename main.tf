resource "aws_instance" "main" {
  ...
  tags = merge(
    {
        Name = "${local.common_name}" # robsohop-dev-catalogue
    },
    local.common_tags
  )
}
...
resource "aws_ami_from_instance" "main" {
  name               = "${local.common_name}-${var.app_version}-${aws_instance.main.id}"   #roboshop-dev-catalogue-v3-instance_id
  source_instance_id = aws_instance.main.id
  depends_on         = [ aws_ec2_instance_state.main ]
  tags = merge(
    {
      Name = "${local.common_name}-catalogue-${var.app_version}-${aws_instance.main.id}"
    },
    local.common_tags
  )
}
